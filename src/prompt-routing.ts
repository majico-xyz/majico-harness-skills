...
const PREFIX_RULES: ReadonlyArray<{ prefix: string; craftSlug: string }> = [
...
const EXACT_RULES: Readonly<Record<string, string>> = {
 ...
 */
export function normalizePromptStem(name: string): string {
 ...
 */
export function craftSkillSlugForPrompt(name: string): string | null {
  const stem = normalizePromptStem(name);
  if (EXACT_RULES[stem]) return EXACT_RULES[stem];
  for (const { prefix, craftSlug } of PREFIX_RULES) {
    if (stem.startsWith(prefix)) return craftSlug;
  }
  if (INVESTOR_PACK_TEMPLATES.has(stem)) return "investor-pack-craft";
  if (MOTION_REVIEW_TEMPLATES.has(stem)) return "motion-review-craft";
  if (CREATIVE_DIRECTION_TEMPLATES.has(stem)) return "creative-direction-craft";
  return null;
}
