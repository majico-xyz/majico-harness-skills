 ...
 */
export function parseHarnessSkillMarkdown(content, fallbackSlug) {
    ...
    const name = typeof frontmatter.displayName === "string"
        ? frontmatter.displayName
        : skillSlug;
    const description = typeof frontmatter.description === "string" ? frontmatter.description : "";
    const backendRef = typeof frontmatter.backendRef === "string" ? frontmatter.backendRef : null;
    return {
        skillSlug,
        name,
        description,
...
export function readHarnessSkillFromDirectory(skillDir) {
    const skillPath = path.join(skillDir, "SKILL.md");
    const content = fs.readFileSync(skillPath, "utf-8");
    return parseHarnessSkillMarkdown(content, path.basename(skillDir));
}
function isHarnessCraftSkill(content) {
    return content.includes("harnessSkill: true");
}
export function listHarnessSkillDirectories(root) {
 ...
 */
export function loadHarnessSkillsFromDisk(root) {
    return listHarnessSkillDirectories(root).map((dir) => readHarnessSkillFromDirectory(dir));
}
/**
 * Build a regex-safe section heading pattern (hyphen or space).
 */
function sectionHeadingPattern(sectionKey) {
    return sectionKey
        .split("-")
